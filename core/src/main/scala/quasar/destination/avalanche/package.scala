...
package object avalanche {
  ...
  def redactedLogEvent(event: LogEvent): LogEvent = {
    def redactAttrValues(s: String) =
      singleQuoted.replaceAllIn(s, "<REDACTED>")
  ...
  def copyVWLoad(
    ...
    val vwloadSql =
      fr"COPY" ++ Fragment.const0(tableName) ++ fr"() VWLOAD FROM" ++
        sourcesSql ++
        fr" WITH" ++
        attrs(authAttrs ::: formatAttrs) ++ fr", AUTO_DETECT_COMPRESSION"

    vwloadSql
      .updateWithLogHandler(LogHandler(logHandler.unsafeRun compose redactedLogEvent))
      .run
  }

  private val singleQuoted: Regex = s"""'[^' ]{2,}'""".r
}
